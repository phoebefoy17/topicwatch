...
def parse_source(entry):
    """Normalize a sources.json entry to (kind, url, match)."""
...
def fetch(limit_per_feed=40):
    ...
    try:
        ...
        print("[warn] trafilatura not installed — falling back to feed "
              "summaries. Install it for real article text.")
    ...
    for bucket, entries in sources.items():
        # Buckets starting with "_" are notes-to-self, not sources.
        # JSON has no comment syntax, so this is how sources.json stays
        # self-documenting.
        if bucket.startswith("_"):
            continue
        for entry in entries:
            kind, url, match = parse_source(entry)
            print(f"  -> [{kind}] {url}" + (f"  match={match}" if match else ""))

            # ---------------- sitemap route ----------------
            if kind == "sitemap":
                if not have_traf:
                    print("     [skip] sitemap sources need trafilatura "
                          "(pip install trafilatura)")
                    continue
                ...
                for u in urls:
                    if new_here >= MAX_NEW_PER_SOURCE:
                        print(f"     [cap] stopping at {MAX_NEW_PER_SOURCE} new "
                              f"— rerun tomorrow to continue backfilling")
                        break
            ...
            for item in parsed.entries[:limit_per_feed]:
                link = item.get("link")
                if not link:
                    continue
                if conn.execute("SELECT 1 FROM articles WHERE url=?",
    ...
    conn.commit()
    print(f"\n[ok] {added} new articles stored. Corpus is now "
          f"{conn.execute('SELECT COUNT(*) FROM articles').fetchone()[0]} docs.")
    return added
...
def rising(recent, baseline, recent_docs, base_docs, min_docs=3, top=25):
    """
...
def report(recent_days=7, baseline_days=45):
    ...
    for n in (2, 3, 1):
        L += [f"## {labels[n]}", "",
              "| term | now | before | lift | score |",
              "|---|---:|---:|---:|---:|"]
        for term, cnt, base, lift, score in rising(rc[n], bc[n], r_n, b_n):
            L.append(f"| {term} | {cnt} | {base} | {lift} | {score} |")
        L.append("")
